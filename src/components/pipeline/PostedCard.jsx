...
const DEST_ACTIVE = {
  'IG Reel':     'bg-pink-900/70 text-pink-100 ring-1 ring-pink-500/60',
  'IG Feed':     'bg-pink-900/70 text-pink-100 ring-1 ring-pink-500/60',
  'IG Carousel': 'bg-pink-900/70 text-pink-100 ring-1 ring-pink-500/60',
  'IG Stories':  'bg-pink-900/70 text-pink-100 ring-1 ring-pink-500/60',
  'TikTok':      'bg-cyan-900/70 text-cyan-100 ring-1 ring-cyan-500/60',
  'YT Shorts':   'bg-red-900/70 text-red-100 ring-1 ring-red-500/60',
  'YT':          'bg-red-900/70 text-red-100 ring-1 ring-red-500/60',
  'FB':          'bg-blue-900/70 text-blue-100 ring-1 ring-blue-500/60',
  'FB Video':    'bg-blue-900/70 text-blue-100 ring-1 ring-blue-500/60',
}
...
export default function PostedCard({ post, isTop, onUpdateStats, onUpdatePost }) {
  const platforms = post.platforms?.length > 0 ? post.platforms : []
  ...
  // Per-platform stats map: { 'IG Reel': { views, likes, ... }, 'TikTok': { ... } }
  const platformStats = post.platform_stats ?? {}

  // Stats to display: active platform's stats (fall back to legacy post.stats for old single-platform posts)
  const activeStats = activePlatform
    ? (platformStats[activePlatform] ?? (!multiPlatform ? post.stats : null) ?? {})
    : (post.stats ?? {})

  // Aggregate across all platforms (for milestone badges)
  const hasPerPlatform = Object.keys(platformStats).length > 0
  const aggregateStats = hasPerPlatform
    ? STAT_FIELDS.reduce((agg, { key }) => {
        agg[key] = Object.values(platformStats).reduce((s, ps) => s + (ps[key] ?? 0), 0)
        return agg
      }, {})
    : post.stats

  const er       = calcER(activeStats)
  const hasStats = activeStats && STAT_FIELDS.some(({ key }) => (activeStats[key] ?? 0) > 0)

  function platformHasStats(pl) {
    const s = platformStats[pl]
    return s && STAT_FIELDS.some(({ key }) => (s[key] ?? 0) > 0)
  }

  function openLogging() {
    setDraft(
      activePlatform
        ? (platformStats[activePlatform] ?? (!multiPlatform ? post.stats : {})) ?? {}
        : post.stats ?? {}
    )
    setLogging(true)
  }
  ...
  function saveStats() {
    ...
    if (activePlatform && platforms.length > 0) {
      const newPlatformStats = { ...platformStats, [activePlatform]: parsed }
      const aggregate = STAT_FIELDS.reduce((agg, { key }) => {
        agg[key] = Object.values(newPlatformStats).reduce((s, ps) => s + (ps[key] ?? 0), 0)
        return agg
      }, {})
      onUpdatePost(post.id, { platform_stats: newPlatformStats, stats: aggregate })
    } else {
  ...
  return (
    <div className={`bg-tac-800 rounded-2xl overflow-hidden transition-all flex flex-col ${
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
          ...
          <div className="ml-auto flex items-center gap-1">
            {dateEdit ? (
              <>
                ...
                <button onClick={saveDate} className="p-1 bg-flo text-tac-950 rounded-lg"><Save size={10} /></button>
                <button onClick={() => setDateEdit(false)} className="p-1 text-tac-400 hover:text-tac-100">✕</button>
              </>
            ) : (
              <button onClick={() => setDateEdit(true)}
        ...
        {/* Platform selector — clickable tabs when multi-platform */}
        {platforms.length > 0 && (
          <div className="flex gap-1 flex-wrap">
            {platforms.map(pl => (
              <button
                key={pl}
                onClick={() => multiPlatform ? switchPlatform(pl) : undefined}
                className={`text-xs px-2.5 py-1 rounded-full font-medium transition-all ${
                  multiPlatform
                    ? activePlatform === pl
                      ? (DEST_ACTIVE[pl] ?? 'bg-flo/20 text-flo ring-1 ring-flo/40')
                      : (DEST_STYLES[pl] ?? 'bg-tac-700 text-tac-100') + ' opacity-50 hover:opacity-100'
                    : (DEST_STYLES[pl] ?? 'bg-tac-700 text-tac-100')
                } ${multiPlatform ? 'cursor-pointer' : 'cursor-default'}`}
              >
                {pl}
                {multiPlatform && platformHasStats(pl) && (
                  <span className="ml-1.5 inline-block w-1.5 h-1.5 rounded-full bg-flo align-middle" />
                )}
              </button>
            ))}
          </div>
        ...
        {/* Active platform label when multi */}
        {multiPlatform && activePlatform && (
          <p className="text-xs text-tac-400">
            Showing stats for <span className="text-tac-100 font-medium">{activePlatform}</span>
            {!platformHasStats(activePlatform) && ' — no stats logged yet'}
          </p>
        )}

        <MilestoneBadges stats={aggregateStats} />
          ...
          <div className="grid grid-cols-3 gap-2">
            {STAT_FIELDS.map(({ key, label }) => (
              <div key={key} className="bg-tac-750 border border-tac-700 rounded-xl p-2.5 text-center">
                ...
                <p className="text-sm font-bold text-stone-100">{fmt(activeStats[key])}</p>
              </div>
            ))}
          </div>
        ) : (
          <button onClick={openLogging}
            ...
            + Log stats{multiPlatform && activePlatform ? ` for ${activePlatform}` : ''}
          </button>
        )}
        ...
        {hasStats && (
          <button
            onClick={() => { if (!logging) openLogging(); else setLogging(false) }}
            className="flex items-center gap-1 text-xs text-tac-300 hover:text-flo transition-colors"
          >
            {logging ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
            {logging ? 'Cancel' : `Update stats${multiPlatform && activePlatform ? ` · ${activePlatform}` : ''}`}
          </button>
        )}

        {/* Logging form */}
        {logging && (
          <div className="space-y-3 pt-1 border-t border-tac-700">
            {multiPlatform && activePlatform && (
              <p className="text-xs font-semibold text-tac-200 pt-2">
                Logging <span className="text-flo">{activePlatform}</span> stats
              </p>
            )}
            <div className="grid grid-cols-3 gap-2">
              {STAT_FIELDS.map(({ key, label }) => (
            ...
            <button onClick={saveStats}
              ...
              Save{multiPlatform && activePlatform ? ` ${activePlatform} Stats` : ' Stats'}
            </button>
          </div>
        )}
